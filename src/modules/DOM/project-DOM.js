...
const ProjectDOM = (() => {
  const generateTab = (projectData) => {
    ...
    addButton.addEventListener('click', ((e) => {
      ...
      PanelDOM.createPanel('Enter todo title:', (value) => {
        ...
        const newTodo = new TodoData(value, '');
  ...
  const toggleSelected = (projectTab, projectData) => {
    const projectTitle = projectTab.children[0];
    const { classList } = projectTitle;
  ...
  const deleteTodoLink = (target, projectID, todo) => {
    ...
    /* Close todo tab if open, remove todo from project
     and delete todo document link from project explorer */
